...
open class BarcodeScannerViewController: UIViewController {
    ...
    /// When the flag is set to `true` controller ignore all equal codes
    public var ignoreEqual = true
...
extension BarcodeScannerViewController {
    func getData(code: QRCode) {
        AF.request("https://tesco-x.herokuapp.com/barcode",
                   method: .post,
                   parameters: code,
                   encoder: JSONParameterEncoder.default).responseJSON { response in
                    ...
                    case .success(let json):
                        ...
                        if let json2 = json as? [String: Any] {
                            guard let qant = Float("\(json2["how_much_will_be_waist"]!)"),
                                let products = json2["products"] as? [[String: Any]],
                                let first = products.first,
                                let description = first["name"] as? String,
                                let image = first["image"] as? String,
                            let url = URL(string: image)
                                else {return}
                            self.cameraViewController.descProduct.text = description
                            self.cameraViewController.iconProduct.af_setImage(withURL: url)
                            let intQ = Int(qant)
                            self.quant = intQ
                            self.cameraViewController.quantLabel.text = "\(intQ)"
                        }
                        //}
    ...
    func addProduct(code: QRCode) {
        AF.request("https://tesco-x.herokuapp.com/catalog/add",
                   method: .post,
                   parameters: code,
